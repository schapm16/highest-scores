...
3. As the score lines are streamed and by using the score as a key, track and store the top N score records - score and jsonData - in an Object. This will give me a data set with no duplicate scores and scores processed later will supersede previous scores of the same value (per the requirements specified).
4. Sort the N tracked scores in descending order.
